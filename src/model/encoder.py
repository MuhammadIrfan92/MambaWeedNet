...
from .modules.module_blocks import Conv_Wavelet_Fusion, MultiGranularityFusion_4VSS
class Encoder(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, kernels=[8, 16, 32, 64, 256]):
        super().__init__()  
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)

        # Encoder 1 Stages (Conv_Wavelet_Fusion: refered to as SAFE in paper)
        self.enc1_1 = Conv_Wavelet_Fusion(in_channels, kernels[0]) # in_channels adjusted for RGB (3) 

        # Stage 2: (MultiGranularityFusion_4VSS: refered to as CAFE in paper) 
        ...
        # Stage 3: (MultiGranularityFusion_4VSS: refered to as CAFE in paper) 
        ...
        # Stage 4: (MultiGranularityFusion_4VSS: refered to as CAFE in paper) 
