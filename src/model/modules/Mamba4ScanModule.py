...
class Mamba4Scan(nn.Module):
    def __init__(self, in_channels, out_channels):

        mamba_in_ch = int(in_channels/d)
        self.mamba_hw_forward = VSSBlock_seldir(mamba_in_ch, direction="hw_forward")
        self.mamba_wh_forward = VSSBlock_seldir(mamba_in_ch, direction="wh_forward")
        self.mamba_hw_reverse = VSSBlock_seldir(mamba_in_ch, direction="hw_reverse")
        self.mamba_wh_reverse = VSSBlock_seldir(mamba_in_ch, direction="wh_reverse")
        self.conv1x1_mamba = Convolution2d1x1_sel(in_channels, out_channels, norm='batch')
    
    def forward(self, x):
        x_hw_forward = self.mamba_hw_forward(x)
        x_wh_forward = self.mamba_wh_forward(x)
        x_hw_reverse = self.mamba_hw_reverse(x)
        x_wh_reverse = self.mamba_wh_reverse(x)

        x_cat = torch.cat([x_hw_forward, x_wh_forward, x_hw_reverse, x_wh_reverse], dim=1)
        out = self.conv1x1_mamba(x_cat)
        return out
